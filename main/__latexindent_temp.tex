...
\(V'\in \text{Pol}(\mathcal{H}_{\Sigma'}) \)
then we can 
give the analogue of \eqref{intro lift condition} in 
...
adapted to the more general notation we have adapted now is
...
\(\mathcal{F}(U^A_{\Sigma',\Sigma}V,\Sigma')\). Furhtermore, we have a 
distinguished polarisation for very early times. Thus, we may 
...
\begin{Corollary}[polarisation classes and lifts, cor 2.7 of \cite{deckert2016perspective}]
...
\begin{Thm}[\(C_{\Sigma}(A)\) depends on \(A|_{T\Sigma}\), thm. 1.5 of \cite{ivp2}]
...
This is aversion of theorem \ref{thm ruisnaar} for general 
Cauchy surfaces.
The next theorem will show how the polarisation classes change 
with the gauge and Lorentz transforms.

\begin{Thm}[transformation of polarisation classes, thm. 1.6 of \cite{ivp2}]
Let \(A\in\mathcal{V}\) be a four-potential and and \(\Sigma\) 
a Cauhcy surface.

\begin{enumerate}[label=\alph*)]
\item Pick \((S,\Lambda)\in \mathbb{C}^{4\times 4}\times \mathrm{SO}^{\uparrow}(1,3)\)
be an orthochronous Lorentz transform, i.e. the tuple fulfills
\(\Lambda^{\mu}_{\, \sigma}g_{\mu \nu}\Lambda^\nu_{\, \tau}=g_{\sigma, \tau}\) 
and \(\quad \Lambda^\mu_{\, \nu}\gamma^\nu = S^{-1} \gamma^\mu S\) 
and has anatural action on wavefunctions (see sec. 2.3 of \cite{ivp1}). 
Then we have
\begin{equation}
...
\todo{move this to after the simple formula to say something about analyticity}
\begin{Remark}
