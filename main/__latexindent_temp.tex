...
As is well known the Dirac equation coupled to an external 
...
space. For furhter details please have a look at section 2 of 
\cite{ivp0}. 
...
if they are in the set \(1+I_1(\mathcal{\ell})\), 
only Dirac seas 
...
If \(P^{V'}U P^V\) is not invertible one can perform the following construction 
...
\end{align}
now we know by theorem \re Ruisnaars \cite{ruisnaar&ivp0} that \(P^+S_{X,Y}P^-\) is a Hilbert-Schmidt operator for our setting,
