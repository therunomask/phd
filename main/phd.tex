...
\todo{read the references on how to find the Greens 
functions and dirac and KG equations on curved spacetime}
...
on the Minkowski half space $\frac{1}{2}\M := [0,\infty) \times 
\R^3$ equipped with the same sign convention as for 
all of Minkowski space.
We focus on retarded Green's functions of the Dirac equation, 
$\mathscr{S}^\ret(x) = \overline{D} G^\ret(x)$ where 
$G^\ret(x)$ is the retarded Green's function of the KG equation. 
Explicitly,
\begin{equation}
	G^\ret(x) = \frac{1}{4\pi} \frac{\delta(x^0-|\vx|)}{|\vx|} - \frac{m}{4\pi} H(x^0-|\vx|) \frac{J_1(m \sqrt{x^2})}{\sqrt{x^2}}
\end{equation}
where $H$ denotes the Heaviside function, $J_1$ a Bessel function.
...
the completion of an algebraic tensor product of Banach spaces depends strongly 
...
\todo{read sources on how to get the greens function here and how the dirac equation looks like on curved spacetime}
...
of the fermion field related to the approach we 
...
We introduce the projectors of the standard polarisation 
of \(\mathcal{H}\) in terms of the hamiltonian of 
the free Dirac equation
\begin{align}
P^-:=1_{\mathrm{spec}(H^0)<0}, \quad P^+=1-P^-,
\end{align}
as the projectors onto the negative, positive energy 
