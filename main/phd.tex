...
subject of controversy. This is the case, despite the 
...
are:
\begin{enumerate}
\item The spectrum of the Hamiltonian generating the 
...
1. In the first approach, we do not introduce a radiation 
...
to transform relativistically we will consider at as a
function of \(N\) spacetime points \(x_i\in\mathbb{R}^4\).
This \emph{multi-time} approach 
goes back to Dirac \cite{dirac_32}, played an important 
...
would be to introduce interaction potentials by minimal 
coupling; however, recent no-go theorems 
\cite{nogo_potentials,deckert_nickel_2016} show that 
no such potentials compatible with Poincaré 
invariance result in an interacting system. 
...
Cauchy surfaces. In the proof of theorem we will see 
that in order to 
...
second requirement we may quickly to through it's proof in 
the one-particle situation. Let \(A,F,G\in\mathcal{V}\) 
such that \(F\prec G\). We may then pick  a Cauchy 
surface \(\Sigma'\) such that \(\supp F\) is causally prior to 
\(\Sigma'\) and \(\Sigma'\) causally prior to \(\supp G\) 
meaning that for \((x,y)\in \supp F \times \Sigma'\) we 
have 
\begin{equation}
  \big((x-y)^2\ge 0\wedge x\neq y\big) \Rightarrow x^0<y^0
\end{equation}
and analogous for \((x,y)\in \Sigma' \times \supp G\).
This implies 
\begin{align}
...
properties of the lift of the time evolution 
...
due to chapter \ref{chap:regularity of S} in the a
appendix.
\end{Remark}
...
\((A+F_1+F_2+G_1+G_2,A+F_1+G_1),(A+F_1+F_2+G_1+G_2,A+F_1+G_1+G_2),
 (A+F_1+F_2+G_1+G_2,A+F_1+F_2+G_1)\in\dom\overline{S}\) holds.
 %\begin{align}
 ...
 %\end{align} 
 %hold. 
 We abbreviate \(A'=A+G_1+F_1\), use \eqref{def z} and compute
...
\begin{equation}
  B_{\mathsf{\Phi},\mathsf{\Xi}}=P^{\mathsf{\Phi}}B P^{\mathsf{\Xi}},
\end{equation}
where \(\mathsf{\Phi},\mathsf{\Xi}\in \{+,-\}\) holds.
...
We define the set 
\begin{equation}
  \mathfrak{B}:=\{B':\mathcal{H}\righttoleftarrow|\mathrm{linear},\|B'\|\in\mathbb{R}, {B'}^*=-B'\}
\end{equation}
and the operator
\begin{equation}
  \mathrm{d}\Gamma(B):=\mathrm{d}\Gamma(B_{++})+\mathrm{d}\Gamma(B_{+-})+\mathrm{d}\Gamma(B_{-+})+\mathrm{d}\Gamma(B_{--}).
\end{equation}
Furhtermore, we endoe \(\mathfrak{B}\) with the topology induced by 
the norm \(\|\cdot\|+\|P^+\cdot P^-\|+\|P^-\cdot P^+\|\).
\end{Def}
...
suffices to show the desired identity on this space. In 
order to do so
we first restrict \(\psi\in \text{span}\{\varphi_{n}| n 
\in \mathbb{Z}\backslash\{0\}\}\). We will do the 
...
the form \todo{is \(\mathcal{F}^0\) really necessary?}
\begin{equation}
...
Nevertheless, the basic protagonists in all three approaches 
...
The first class of objects of interest are Hadamard states 
which appear in the algebraic approach to 
QFT for a historyic reference see \cite{fulling1978singularity}. 

The second class of objects are the (\(I_2\)-almost) 
projectors \(P^\lambda_{\Sigma}\) which are 
operators that are Hilbert-Schmidt 
close to projectors onto polarisation classes 
discussed in section \ref{sec: qft intro}.
...
\(\Sigma, \Sigma'\) denote arbitrary Cauchy surfaces. 
We choose 
...
such that for all \(a\in\mathbb{N}\) there are \(C_a>0\) such that for 
...
\begin{equation}
  \mathrm{WF}(v):= \left\{(x,\xi;y,\xi')\in \mathbb{R}^n\times \mathbb{R}^n\mid (\xi,\xi') \in \Xi_{x,y}(v) \right\}.
\end{equation}
\end{Def}
...
\begin{Def}
The Klein-Gordon operator corresponding to the Dirac equation \eqref{dirac ext field} reads
\begin{align}
...
the Hadamard states \(H \in (\mathcal{S}(\mathcal{M})\times \mathcal{S}(\mathcal{M}))'\)  on which we base our analysis below.
According to this, \(H\) 
...
The functions \(V_k\) fulfil a recursive set of partial 
...
The concept of polarisation classes introduced in 
...
The following theorem is the basis of our comparison 
between\linebreak Hadamard states and \(I_2-\) almost Projectors. 
We first
discuss its consequences and postpone the proof until 
the appendix in section \ref{sec: proof hadamard}.


\begin{Thm}\label{thm:hadamard=>Pol}
Given as four-potential \(A\in \mathcal{V}\), and a Hadamard state \(H\) of the form 
...
While this is more conventional than what was 
...
Subsequently, we gave a geometric construciton of 
...
weak fields. Finally we gave theorems on how 
\(I_2-\)almost projectors representing the state 
of the fermion field and related to the approach we 
are following here compares to the concept of Hadamard 
...
the fermions and thus be a further step in the direction 
...
relation of the form \eqref{Commutation Gamma}, we calculate \(\left[ T_m-\Gamma_m, a^\#(\varphi_n) \right]\)
...
\subsection{Proofs of Section \ref{sec: hadamard}}\label{sec: proof hadamard}
...
\((y-x)^2=0\), if we further demand that both \(x\) and 
\(y\) belong to the same spacelike hypersurface \(x=y\) 
follows. This implies that for functions 
...
We define
\begin{align}
&g_1: \Sigma\times\Sigma\times ]0,1[\rightarrow \mathbb{C}^{4\times 4}\\
...
\begin{equation}
 \forall (x,y,\varepsilon)\in J_3\times]0,1[ :|g_1(x,y,\varepsilon)|\le M_1.
\end{equation}
Furthermore, there is a function 
\(g_3 \in C(\Sigma\times\Sigma,\mathbb{R}^+)\) such that
\begin{equation}
\forall (x,y,\varepsilon)\in \dom(g_2): |g_2(x,y,\varepsilon)|\le g_3(x,y)
\end{equation}
holds.
\end{Lemma}
\begin{proof}
...
(x-y)^\alpha &\int_0^1 \mathrm{d}s (1-2s) (\slashed{\partial}A_\alpha)(sx+(1-s)y)=
  \\\notag
& -\frac{(x-y)^\alpha}{6}(x-y)^\beta  (\slashed{\partial}\partial_\beta A_\alpha)((x+y)/2) + \mathcal{O}(\|x-y\|^3).
\end{align}
...
& + \sup_{\varepsilon \in ]0,1]}|w_\varepsilon|(x,y)
\end{align*}
