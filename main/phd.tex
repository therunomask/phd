...
Pick \(A,F\in\mathcal{V}\) and \(t_1,t_0\) as in the theorem. 
...
R^{A+F}(t_1,t_0):=[(1-Q^{A+F})U^{A+F}(1+Q^{A+F})](t_1,t_0)=U^{A+F}(t_1,t_0).
\end{equation}

So it suffices to study the family of operators \(R\). 
Form the analysis carried out in \cite{ivp0} we know that 
\(R^B\) for \(B\in\mathcal{V}\) is the limit in the sense of the operator norm of 
the sequence
\begin{align}
R^B_{(0)}:=0, \quad R^B_{(n+1)}:=U^0 F^B R^B_{(n-1)}+ U^0 + G^B,
\end{align}
where \(F\) and \(G\) are given By

\begin{align}
F^B:= (-{Q'}^B+Z^B_{\mathrm{ev}}-Q^BZ^B)(1+Q^B)\\
G^B:=- U^0(1-Q^BQ^B) + U^0(-{Q'}^B+Z_{\mathrm{ev}}-Q^B Z^B)Q^B Q^B U^B (1+Q^B)\\
Z^B_{\mathrm{ev}}=P^+ Z^B P^++P^- Z^B P^-
Z^B_N{\mathrm{odd}}=Z^B-Z^B_{\mathrm{ev}}.
\end{align}
