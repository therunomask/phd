...
\todo{write recursive equation for U, introduce R,F,G etc.}
...
\end{equation}
which according to lemma \ref{lem properties of dom s bar} is open and
fulfills that \(\bigcup_{x\in\mathbb{R}} U_x\times U_x\) is an open 
neighbourhood of the diagonal \(\{(x,x)\mid x \in\mathbb{R}\}\).
Furthermore for any \(x\in \mathbb{R}\) the set \(U_x\) is connected, 
as \(\dom\overline{S}\) is star-shaped in the sense of property
4 of lemma \ref{lem properties of dom s bar}.
...
The proof is by induction over the domain of definition of our 
extention of \(\phi_A\). 
The induction hypothesis is that \(\psi_n\) defined
on  \(\dom \psi_n=\dom_n \times\dom_n\). The set 
\(\dom_n\) is any open interval of the form 
\begin{equation}
\bigcup_{k=0}^n U_{x_k},
\end{equation}
with \((x_k)_k\) such that 
\begin{align}
  \forall k\le n: U_{x_k} \cap \bigcup_{l=0}^k U_{x_l}\neq \emptyset\\
  \forall k\le n: U_{x_k} \cap 
  \left(\bigcup_{l=0}^k U_{x_l}\right)^c\neq \emptyset,
\end{align}
holds. Moreover \(\psi_n\) satisfies the conditions
...
\end{align}
and any other function fulfilling properties 
\eqref{induction psi1}-\eqref{induction psi3} possibly being defined
on a larger domain coincides with \(\psi_n\) on \(\dom_n\).

We start with \(\psi_0=\phi_A\)  
restricted to \(U_{x_0} \times U_{x_0}\) for some \(x_0\in\mathbb{R}\).
This function is an extension of \(\phi_A\) and because of 
lemma \ref{lem phi local} it 
fulfils all of the required properties directly. 
 
For the induction step we pick 
\(z\) such that 
\begin{align}
U_{z} \cap \dom_n \neq \emptyset\\
U_{z} \cap \dom_n^c\neq \emptyset
\end{align}
hold and define the open interval \(U_{z}\cup \dom_{n+1}\).
...
with \(\psi_n\) replaced by \(\psi_{n+1}\)  and is the unique function 
to do so.
...
\begin{equation}
 \partial_{\varepsilon_1}\partial_{\varepsilon_2}\ln  z(A+\varepsilon_1 H,A+\varepsilon_2 K)=\partial_{\varepsilon_1}\partial_{\varepsilon_2}\ln\Gamma_{A,A+\varepsilon_1 H,A+\varepsilon_2 K}\overset{\eqref{gamma attri8}}{=}c_A(H,K). 
\end{equation}
