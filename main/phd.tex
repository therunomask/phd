...
The first one considers a recent relativistically invariant 
integral equation for multi-time wavefunctions 
by Lienert\cite{direct_interaction_quantum}. This is an 
equation for fermions interacting directly along light-like 
configurations. In this thesis firstly, previous 
...
As a first step in the direction of an interacting theory, we
introduce external electrodynamic fields into otherwise free 
quantum field theory. In previous results, candidates for 
the time evolution operator have been constructed in this setting.
This construction is unique up to a global phase, which may 
depend on the external field. In this work, the problem of 
identifying the phase of the resulting scattering operator 
is partially addressed by a geometric 
construction incorporating a certain causality condition.

Secondly a simple formula for the  
...
how to construct the central object of one approach from the
...
person, nor does it refer to me and my 
...
individual mathematical expressions that appear in previous equations or 
...
in round brackets. Contrary to standard practice, we will also 
use these abbreviations in formulas. If we want to emphasise 
...
The history of relativistic quantum mechanics since the 
...
in 1928 \cite{dirac1928quantum} is long and complicated. 
One reason for this is the 
extraordinary success with which Quantum Field Theory (QFT)
has been applied to predict the outcomes of experiments 
at particle accelerators such as the 
large hadron collider. Another reason is that there is 
still no mathematically rigorous theory forming a 
solid foundation of those Quantum Field Theories. 
The free theory; however, is well understood see e.g. 
\cite{derezinski2013mathematics} for an overview of the 
rigorous mathematical results. Bearing this in mind, we 
might ask ourselves what kinds of interaction can be 
rigorously defined in a relativistic quantum mechanical 
setting. We are going to describe aspects of this 
question in detail.

\begin{enumerate}
\item In describing the first faced, we will introduce 
interaction into a system of persistent particles.
Analogously to non-relativistic quantum mechanics, 
we will introduce a wavefunction 
...
for the wavefunction of the particles. The result is the equation 
...
\end{align}
of Lienert\cite{direct_interaction_quantum} for spin 1/2 particles. 
Equations like 
...
the previous results about related equations are summarised in section 
\ref{sec: direct interation previous}.

There are two main results of this thesis about equations of the 
...
quantum electrodynamics without photons has only 
...
provide a first step to mutual intelligibility between 
the algebraic formulation of QFT and the formulation 
used in this thesis by providing a means to translate 
the central object of one formulation to the central 
object of the other.
\end{enumerate}
...
research of   Lienert and the author of this thesis. 
...
It acts on wavefunctions as 
\begin{equation}
  H^\free \phi(t,\vx)=i \gamma^0 \vec{\gamma} \cdot \nabla \phi + m \gamma^0 \phi.
\end{equation}
