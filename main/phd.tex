...
We begin by reformulating the Schr{\"o}dinger equation for one particle as an integral equation. 
Extending this equation to two particles and imposing Poincar\'{e} invariance naturally leads us to an 
integral equation for a multi-time wave function.

Consider the one particle  Schr{\"o}dinger equation for the wave function $\varphi(t,\vx)$:
\begin{equation}
	i \partial_t \varphi(t,\vx) = \big( H^\free + V(t,\vx) \big) \varphi(t,\vx),
	\label{eq:singlepartschroed}
\end{equation}
where $H^\free$ denotes the non interacting Hamiltonian of the Schrödinger equation.\\
We denote by $G^\ret(t,\vx)$ the retarded Green's function of the non interacting equation, i.e.
\begin{equation}
	\big( i \partial_t - H^\free \big) G^\ret(t,\vx) = \delta(t) \delta^{(3)}(\vx)
	\label{eq:schroedgreensfn}
\end{equation}
with $G^\ret(t,\vx) = 0$ for $t< t_0$.
Then the initial value problem induced by \eqref{eq:singlepartschroed} for $t\geq t_0$ with $\varphi(t_0,\vx) = \varphi_0(\vx)$ is equivalent to the integral equation (see \cite{feynman1949theory})
\begin{equation}
...
\begin{itemize}
  ...
  The factor $i$ is necessary to obtain the correct non-relativistic limit 
