...
\left[\begin{matrix}n\\p\end{matrix}\right]\\\label{B++ short estimate}
    &\times \langle B^{L\cup\{n\}}_{n} \tilde{\varphi}_{n}, B^{L\cup\{p\}}_{p}\tilde{\varphi}_{p}\rangle\\
  &+\sum_{\overset{L,C\subset \mathbb{N}}{|L|=m,|C|=p}}\hspace{-0.4cm} |\alpha_{L,C}|^2
   \sum_{b,b'=1}^m 
  \left\langle \prod_{l=1}^{b-1} a^*(\tilde{\varphi}_{L_l})  ~~ a^*(B^{L}_{L_b} \tilde{\varphi}_{L_b})\right.  \\\nonumber
  &\prod_{l=b+1}^m a^*(\tilde{\varphi}_{L_l}) \Omega, 
...
\textbf{\(1\):} The first term 
is estimated after rewriting the scalar products as a 
...
\textbf{\(\delta_{n_1,p_1}\):} 


What remains is the second sum in \eqref{B++ short estimate}, for this term there are two cases.  If \(b=b'\) then the scalar product is 
...
and fulfils the \eqref{lift condition}. Because these conditions uniquely fix the Operator \(S^A\)  up to a phase this suffices as a proof.
