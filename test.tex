...
\end{equation}
 for suitable \(g\) and \((\gamma_l)_l\) produce multiples of \(K(m_1,m_2)\)? We will call this number of 
...
F(g,k)= g^k -  \sum_{n=1}^{g-1} \binom{g}{n} F(n,k).
\end{equation}
...
\end{equation}
for external potentials small enough, where \(C_n\) must be imaginary for any \(n\in\mathbb{N}\) in order to satisfy unitarity.
For smallness the following condition is sufficient:
\begin{equation}
\|\id-U^A\|<1.
\end{equation}
\end{Thm}
